...
//-test-
        ...
        Mat s = Mat::zeros(height, height * width, CV_32FC1);
        Mat e = Mat::zeros(height, height * width, CV_32FC1);


        Mat train_images = Mat::zeros(number_of_images, height * width, CV_32FC1);
        ...
        for (int i = 0; i < number_of_images; i++) {
            for (int j = 0; j < height * width; j++) {
                ...
                train_images.at<float>(i, j) = pixel_value;

                //打印第一张和最后一张图像数据
                if (i == 0) {
                    s.at<float>(j / width, j % width) = pixel_value;
                } else if (i == number_of_images - 1) {
                    e.at<float>(j / width, j % width) = pixel_value;
                }
            }
        }

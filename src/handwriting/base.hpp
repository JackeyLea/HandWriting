 ...
 * @description including any actions ,any methods aims to process image like gray binary rgb2bgr
 * and ect.
 */
...
inline void displayImage(QLabel *label, Mat srcImage) {
    ...
    QImage img = QImage(static_cast<const unsigned char *>(srcImage.data), srcImage.cols,
                        srcImage.rows, srcImage.cols * srcImage.channels(), QImage::Format_RGB888);
...
inline Mat dilateImage(Mat inputImage, int expand_type = MORPH_RECT, int expand_size = 1) {
    Mat tempImage;
    Mat element = getStructuringElement(expand_type, Size(expand_size, expand_size), Point(-1, -1));
...
inline Mat deskewAndCrop(Mat inputImage, const RotatedRect &box) {
    ...
    if (angle < -45.0) {
        angle +=
            90.0; //角度小于45度，意味着文本是垂直对齐的，增加90度的旋转角度，交换宽度与高度属性
    ...
    Mat cropped; //裁剪结果
    // void getRectSubPix(InputArray image, Size patchSize, Point2f center, OutputArray patch, int
    // patchType=-1 )
...
inline QImage cvMatToQImage(const cv::Mat &inMat) {
    switch (inMat.type()) {
        ...
        case CV_8UC1: {
            ...
            QImage image(inMat.data, inMat.cols, inMat.rows, static_cast<int>(inMat.step),
                         QImage::Format_Grayscale8); // Format_Alpha8 and Format_Grayscale8 were
                                                     // added in Qt 5.5
#else //这里还有一种写法，最后给出
...
inline cv::Mat QImageToCvMat(const QImage &inImage, bool inCloneImageData = true) {
    switch (inImage.format()) {
        ...
        case QImage::Format_RGB888: {
            if (!inCloneImageData) {
                qWarning() << "CVS::QImageToCvMat() - Conversion requires cloning because we use a "
                              "temporary QImage";
            }
